...
class WaymoDataset(data.Dataset):
    ...
    def __init__(self, data_path, voxel_size, imageset='train', init_lst=None):
        ...
        if imageset == 'train':
            split = waymoyaml['split']['train']
        elif imageset == 'val':
            split = waymoyaml['split']['valid']
        elif imageset == 'test':
            split = waymoyaml['split']['test']
        elif imageset == 'active-label':
            with open('dataloader/waymo/init_data/init_label_scan.json') as f:
                lst = json.load(f)
        elif imageset == 'active-ulabel':
            with open('dataloader/waymo/init_data/init_ulabel_scan.json') as f:
                lst = json.load(f)
...
if __name__ == "__main__":
    dataset = WaymoDataset("/work/patrickwu2/PCL_Seg_data/SemanticKitti/sequences", voxel_size=0.05)
