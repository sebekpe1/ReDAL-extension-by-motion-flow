...
class RegionWaymo(data.Dataset):
    def __init__(self, data_path, voxel_size, imageset='train', init_lst=None, mode='ORG', pseudolabels_folder=None):
        ...
        self.mode = mode
        self.pseudolabels_folder = pseudolabels_folder
    ...
    def __getitem__(self, index):
        ...
        if self.imageset == 'active-ulabel':
            ...
            assert len(preserving_labels) != 0, f'Zero annotated supervoxel in frame:{self.im_idx[index]} supvox[frame]: {preserving_labels}'
            mask = np.isin(supvox, preserving_labels)
            all_labels = np.where(mask, all_labels, 255)

            # Pseudolabels
            # pseudolabels_mask = ~mask * (supvox != 0)
            pseudolabels_supvoxels = np.arange(0, np.max(supvox) + 1)
            pseudolabels_supvoxels = np.delete(pseudolabels_supvoxels, np.append(0, preserving_labels).astype(int))
            pseudolabels_supvoxels = np.random.choice(pseudolabels_supvoxels, int(len(pseudolabels_supvoxels)/40), replace=False)
            pseudolabels_mask = np.isin(supvox, pseudolabels_supvoxels)
            pseudolabel_file = self.im_idx[index].replace('velodyne', self.pseudolabels_folder).replace('.bin', '.label')
            if os.path.exists(pseudolabel_file):
                pseudolabels = np.fromfile(pseudolabel_file, dtype=np.int32).reshape(-1)
                all_labels = np.where(pseudolabels_mask, pseudolabels, all_labels)
