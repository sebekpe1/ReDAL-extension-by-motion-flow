...
class ReDALSelector:
    ...
    def __init__(self, batch_size, num_workers, config_path, pseudo_label_folder):
        ...
        with open(config_path, "r") as f:
            print(f'Hard coded Semantic KITTI config file')
            self.config = argparse.Namespace()
            self.config.num_clusters = 150
            self.config.alpha = 1.0
            self.config.beta = 0.0
            self.config.gamma = 0.05
            self.config.decay_rate = 0.95
            self.config.trim_region = True
            self.config.trim_rate = 0.1
        ...
        with torch.no_grad():
            for i_iter_test, batch in enumerate(tqdm_loader):
                ...
                for batch_idx in range(self.batch_size):
                    ...
                    pseudolabels = np.argmax(output.cpu().numpy(), axis=1)

                    pseudolabels_path = fname.replace('velodyne', self.pseudo_label_folder).replace('.bin', '.label')
                    os.makedirs('/'.join(pseudolabels_path.split('/')[:-1]), exist_ok=True)
                    pseudolabels.astype(np.int32).tofile(pseudolabels_path)
