...
# custom
from base_agent_weighted_batch import BaseTrainer
...
def main(rank, args):
    ...
    for selection_iter in range(1, args.max_iterations + 1):
        ...
        # evaluate the result
        val_return = trainer.validate(update_ckpt=False, print_recall_data=True)
        if rank == 0:
...
if __name__ == '__main__':
    ...
    parser.add_argument('-l', '--pseudo_label_folder', type=str, default=None, help='folder to save pseudo_labels')
    ...
    parser.add_argument('--mode', choices=['ORG', 'GT', 'MF'], default='ORG', help='Original (ORG), ground truth (GT) '
                                                                                   'instances, motion flow (MF) '
                                                                                   'instances')
