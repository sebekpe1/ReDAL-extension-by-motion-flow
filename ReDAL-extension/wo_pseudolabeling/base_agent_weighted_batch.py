...
class BaseTrainer(object):
    def __init__(self, args, logger):
        ...
        self.loss_fun = torch.nn.CrossEntropyLoss(ignore_index=self.args.ignore_idx, reduction='none')
    ...
    def train_one_epoch(self, epoch, validation):
        ...
        for i_iter, batch in enumerate(self.train_dataset_loader):
            ...
            loss_weights = torch.zeros(loss.shape[0])
            start_idx = 0
            for batch_idx in range(self.args.train_batch_size):
                batch_num_vox = torch.count_nonzero(batch['targets'].C[:, -1] == batch_idx)
                batch_labels = targets.cpu()[start_idx:start_idx + batch_num_vox]
                loss_weights[start_idx:start_idx + batch_num_vox] = torch.count_nonzero(batch_labels != self.args.ignore_idx) / batch_labels.shape[0]
                start_idx += batch_num_vox
