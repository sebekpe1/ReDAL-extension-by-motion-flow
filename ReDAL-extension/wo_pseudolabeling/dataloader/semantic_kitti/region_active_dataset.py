...
class RegionActiveSemKITTI:
    def __init__(self, args):
        ...
        self.label_dataset = RegionSemKITTI(args.data_dir, imageset='active-label', voxel_size=0.05, mode=self.args.mode)
        self.pool_dataset = RegionSemKITTI(args.data_dir, imageset='active-ulabel', voxel_size=0.05, mode=self.args.mode)
        ...
        if self.args.mode == 'ORG':
            ...
            with open("dataloader/semantic_kitti/init_data/semkitti_large_pts_GT.json", 'r') as f:
                self.supvox_pts = json.load(f)
            with open("dataloader/semantic_kitti/init_data/dynamic_instances_GT.txt", 'r') as f:
                ...
                for d_i in self.dynamic_instances_dict.keys():
                    if dict_name in self.dynamic_instances_dict[d_i]:
                        for add_supvox in self.dynamic_instances_dict[d_i]:
                            ...
                            add_supvox_id = add_supvox.split('#')[1]
