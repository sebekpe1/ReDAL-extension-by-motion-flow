def get_dataset(name, data_root, imageset, voxel_size=0.05, init_lst=None):
    ...
    from dataloader.waymo.dataset import WaymoDataset
    if name == 's3dis':
        ...
        dataset = WaymoDataset(data_root, voxel_size, imageset=imageset, init_lst=init_lst)
    elif name == 'scannet':
...
def get_active_dataset(args, mode='scan'):
    if mode == 'scan':
        ...
        from dataloader.waymo.region_active_dataset import RegionActiveWaymo
        ...
        elif args.name == 'waymo':
            dataset = RegionActiveWaymo(args)
        elif args.name == 'scannet':
