...
if __name__ == '__main__':
    ...
    with open("output/MF_dynamic_instances_waymo.txt", "r") as f:
    ...
    for seq_idx in work:
        ...
        print(f'\r{seq_idx}/{len(work)} --> {100 * seq_idx/len(work):.01f}%', end='')

        instances_paths = sorted(glob.glob(f'{dataset_path}/{seq_idx:04d}/annotator_instances/*.label'))

        os.makedirs(f'{dataset_path}/{seq_idx:04d}/supervoxel_MF/', exist_ok=True)
        ...
        for instances_path in instances_paths:
            ...
            supervoxel_path = instances_path.replace('annotator_instances', 'supervoxel').replace('.label', '.bin')
            ...
            if len(np.unique(dynamic_instances)) == 1:
                ...
                save_supervoxel = open(f'{dataset_path}/{seq_idx:04d}/supervoxel_MF/{frame}.bin', 'wb')
            ...
            for inst, supvox_id in zip(added_instances, added_instances_supvox_id):
                ...
                dynamic_instances_dict[inst].append(f'{seq_idx:04d}_{frame}#{int(supvox_id)}')

            save_supervoxel = open(f'{dataset_path}/{seq_idx:04d}/supervoxel_MF/{frame}.bin', 'wb')
            save_supervoxel.write(supervoxels.astype(np.uint32))
            save_supervoxel.close()

    with open('output/dynamic_instances.txt', 'w') as f:
        json.dump(dynamic_instances_dict, f)
    print(f'\nDictionary with dynamic instances and their supervoxels (dynamic_instances) was saved')

    with open('output/scene_instances.txt', 'w') as f:
